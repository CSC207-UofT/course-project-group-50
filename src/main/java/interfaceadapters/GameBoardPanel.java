...
public class GameBoardPanel extends JPanel {
    ...
    private void drawGridLines(Graphics g) {
        ...
        for(int i = 0; i < 7; i++) {
            g.drawLine(i * Presenter.WIDTH / 7, 0, i * Presenter.WIDTH / 7,  Presenter.HEIGHT / 7);
        }
        // line to close boundary of squares
        g.drawLine(0, Presenter.HEIGHT / 7, Presenter.WIDTH, Presenter.HEIGHT / 7);
        // draw grid lines on bottom row
        for(int i = 0; i < 7; i++) {
            g.drawLine(i * Presenter.WIDTH / 7, Presenter.HEIGHT,
                    i * Presenter.WIDTH / 7,  Presenter.HEIGHT - (Presenter.HEIGHT / 7));
        }
        // line to close boundary of squares
        g.drawLine(0, Presenter.HEIGHT - (Presenter.HEIGHT / 7), Presenter.WIDTH,
                Presenter.HEIGHT - (Presenter.HEIGHT / 7));
        //draw grid lines on left side
        for(int i = 1; i < 6; i++) {
            g.drawLine(0, i * Presenter.HEIGHT / 7, Presenter.WIDTH / 7,  i * Presenter.HEIGHT / 7);
        }
        // line to close boundary of squares
        g.drawLine(Presenter.WIDTH / 7, 0, Presenter.WIDTH / 7, Presenter.HEIGHT);
        // draw grid lines on right side
        for(int i = 1; i < 6; i++) {
            g.drawLine(Presenter.WIDTH, i * Presenter.HEIGHT / 7,
                    Presenter.WIDTH - (Presenter.WIDTH / 7),  i * Presenter.HEIGHT / 7);
        }
        // line to close boundary of squares
        g.drawLine(Presenter.WIDTH - (Presenter.WIDTH / 7), 0,
                Presenter.WIDTH - (Presenter.WIDTH / 7), Presenter.HEIGHT);
    }
